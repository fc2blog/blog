...
$(function(){
  ...
  // ajaxで一覧情報をロード
  $('#sys-ajax-files-index').load('<?php echo \Fc2blog\Web\Html::url($request, array('action'=>'ajax_index')); ?>')
});
...
function ajaxSubmit(url){
  ...
  $.ajax({
    url: '<?php echo \Fc2blog\Web\Html::url($request, array('action'=>'ajax_index')); ?>',
    type: 'GET',
    data: formData,
    success: function(res){
      $('#sys-ajax-files-index').html(res);
