...
<dl class="output_contents comment_about">
  ...
  <dd class="about_content"><a href="<?php echo \Fc2blog\App::userURL($request,array('controller'=>'entries', 'action'=>'view', 'blog_id'=>$comment['blog_id'], 'id'=>$comment['entry_id'], 'sp'=>1)); ?>" target="_blank"><?php echo $comment['entry_title']; ?></a></dd>
</dl>
...
<div class="btn_area">
  ...
  <ul class="btn_area_inner">
    <li><a class="btn_contents touch" href="<?php if($request->isArgs('back_url')): ?><?php echo $request->get('back_url'); ?><?php else: ?><?php echo \Fc2blog\Web\Html::url($request, array('action'=>'index')); ?><?php endif; ?>"><i class="return_icon btn_icon"></i><?php echo __('I Back to List'); ?></a></li>
    <li><a class="btn_contents touch" href="<?php echo \Fc2blog\Web\Html::url($request, array('action'=>'delete', 'id'=>$comment['id'], 'back_url'=>ue($request->get('back_url')))); ?>" onclick="return confirm('<?php echo __('Are you sure you want to delete?'); ?>');"><i class="delete_icon btn_icon"></i><?php echo __('Delete'); ?></a></li>
  </ul>
</div>
