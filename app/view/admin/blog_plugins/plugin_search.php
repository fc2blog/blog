  ...
  <tbody>
    <?php foreach($plugins as $plugin): ?>
      <tr>
        ...
        <td class="center">
          <a href="<?php echo \Fc2blog\Web\Html::url(array('action'=>'download', 'id'=>$plugin['id'], 'category'=>$request->get('category'), 'sig'=>\Fc2blog\Web\Session::get('sig'))); ?>"><?php echo __('Download'); ?></a>
        </td>
        <td class="center">
          <a href="<?php echo \Fc2blog\App::userURL(array('controller'=>'entries', 'action'=>'preview', 'blog_id'=>$this->getBlogId(), 'plugin_id'=>$plugin['id'], 'category'=>$request->get('category'), $device_key=>1), false, true); ?>" target="_blank"><?php echo __('Preview'); ?></a>
        </td>
        <td class="center">
          <?php if ($user_id==$plugin['user_id']): ?>
            <a href="<?php echo \Fc2blog\Web\Html::url(array('action'=>'plugin_delete', 'id'=>$plugin['id'], 'sig'=>\Fc2blog\Web\Session::get('sig'))); ?>" onclick="return confirm('<?php echo __('Are you sure you want to delete?'); ?>');"><?php echo __('Delete'); ?></a>
          <?php else: ?>
            &nbsp;
          <?php endif; ?>
        </td>
