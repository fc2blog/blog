      ...
      <?php foreach($blog_plugins as $blog_plugin): ?>
        <li class="link_list_item">
          ...
          <div class="switch_checkbox" style="display: none;">
            ...
            <input type="checkbox" value="<?php echo $blog_plugin['id']; ?>" class="toggle" <?php if ($blog_plugin['display']==\Fc2blog\Config::get('APP.DISPLAY.SHOW')) echo 'checked="checked"'; ?> />
          </div>
  ...
  $('#btn_area_display button').on('click', function(){
    ...
    $('ul.link_list input[type=checkbox]').each(function(){
      form.append('<input type="hidden" name="blog_plugins['+$(this).val()+']" value="'
        +($(this).prop('checked') ? <?php echo \Fc2blog\Config::get('APP.DISPLAY.SHOW'); ?> : <?php echo \Fc2blog\Config::get('APP.DISPLAY.HIDE'); ?>)+'" />');
    });
    form.submit();
  });
