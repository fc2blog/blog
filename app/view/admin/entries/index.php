  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="m_cell">
          <a href="<?php echo \Fc2blog\Model\BlogsModel::getFullHostUrlByBlogId($this->getBlogId(), \Fc2blog\Config::get('DOMAIN_USER')); ?><?php echo \Fc2blog\App::userURL(array('controller'=>'Entries', 'action'=>'view', 'blog_id'=>$entry['blog_id'], 'id'=>$entry['id'])); ?>" target="_blank">
            <?php echo th($entry['title'], 10); ?>
          </a>
        </td>
        <td><?php echo th($entry['body'], 10); ?></td>
        <td class="center s_cell"><a href="<?php echo \Fc2blog\Web\Html::url(array('action'=>'edit', 'id'=>$entry['id'])); ?>"><?php echo __('Edit'); ?></a></td>
        <td><?php echo $open_status_list[$entry['open_status']]; ?></td>
        <td>
          <?php if ($entry['comment_count']>0): ?>
            <a href="<?php echo \Fc2blog\Web\Html::url(array('controller'=>'Comments', 'action'=>'index', 'entry_id'=>$entry['id'])); ?>"><?php echo $entry['comment_count']; ?></a>
          <?php else: ?>
            <?php echo $entry['comment_count']; ?>
          <?php endif; ?>
        </td>
        <td class="center s_cell"><a href="<?php echo \Fc2blog\Web\Html::url(array('action'=>'delete', 'id'=>$entry['id'], 'sig'=>\Fc2blog\Web\Session::get('sig'))); ?>" onclick="return confirm('<?php echo __('Are you sure you want to delete?'); ?>');"><?php echo __('Delete'); ?></a></td>
      </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
