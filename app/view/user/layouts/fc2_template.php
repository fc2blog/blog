...
if (!empty($entries)) {
  foreach ($entries as $key => $value) {
    ...
    $entries[$key]['link'] = \Fc2blog\App::userURL($request,array('controller'=>'Entries', 'action'=>'view', 'blog_id'=>$value['blog_id'], 'id'=>$value['id']));
...
if (!empty($comments)) {
  foreach ($comments as $key => $value) {
    $comments[$key]['edit_link'] = \Fc2blog\Web\Html::url($request, array('controller'=>'Entries', 'action'=>'comment_edit', 'blog_id'=>$value['blog_id'], 'id'=>$value['id']));
