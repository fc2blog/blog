...
class UserServiceTest extends TestCase
{
    public function testUpdatePassword(): void
    {
        ...
        $user->setPassword('testadmin1');
        UserService::update($user);

        $user_new = UserService::getByLoginId($login_id);
        $this->assertNotEquals($old_pass_hash, $user_new->password);

        $this->assertTrue($user_new->verifyPassword('testadmin1'));
        $this->assertFalse($user_new->verifyPassword('testadmin'));

        UserService::updatePassword($user, 'testadmin');
    }
}
